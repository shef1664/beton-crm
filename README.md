...
```
АВТОМАТИЗИРОВАННЫЙ_ОТДЕЛ_ПРОДАЖ/
├── backend/
...
Формула расчёта:
```
Цена доставки за 1 м³ = (Расстояние × 35 ₽) + 650 ₽
Оплачиваемый объём доставки = max(6 м³, объём заказа с округлением вверх)
Стоимость = (Цена_марки × Объём) + (Цена доставки за 1 м³ × Оплачиваемый объём доставки)
```

Пример:
- 1.5 м³ М300 на 7 км
- бетон: 1.5 × 7200 = 10 800 ₽
- доставка за 1 м³: 7 × 35 + 650 = 895 ₽
- оплачиваемый объём доставки: 6 м³
- доставка: 895 × 6 = 5 370 ₽
- итого: 16 170 ₽
