...
// ===== DISPATCH STATUS =====
const dispatchWindows = ['60-90 мин', '75-105 мин', '90-120 мин', 'сегодня 16:30', 'сегодня 18:00'];
let dispatchWindowIndex = 0;

function updateDispatchWindow() {
    const el = document.getElementById('dispatch-window');
    if (!el) return;
    dispatchWindowIndex = (dispatchWindowIndex + 1) % dispatchWindows.length;
    el.textContent = dispatchWindows[dispatchWindowIndex];
}

setInterval(updateDispatchWindow, 45000);
...
function getTrackingParams() {
    ...
    return {
        ...
        source_campaign: params.get('campaign') || params.get('utm_campaign') || 'dispatch-window',
        source_listing: window.location.pathname,
    };
}
...
if (speedForm) {
    speedForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const volume = parseFloat(data.volume) || 5;
            const grade = data.concrete_grade || 'М200';
            const address = String(data.address || '').trim();
            const urgency = data.urgency || 'today';
            ...
            const response = await fetch(`${API_URL}/api/leads/create`, {
                ...
                body: JSON.stringify({
                    ...
                    source_listing: tracking.source_listing,
                    source_campaign: tracking.source_campaign,
                    utm_source: tracking.utm_source,
                    utm_medium: tracking.utm_medium,
                    utm_campaign: tracking.utm_campaign,
                    client_type: 'private',
                    next_action: 'Проверить окно подачи и стоимость доставки',
                    comment: `Лендинг-диспетчерская: ${grade}, ${volume} м³${address ? ', объект: ' + address : ''}`
                })
            });

            const result = await response.json().catch(() => ({}));
            if (response.ok && (result.status === 'success' || result.status === 'duplicate')) {
                alert('✅ Заявка принята! Диспетчер проверит окно подачи и перезвонит.');
                speedForm.reset();
            } else {
