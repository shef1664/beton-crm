...
Backend уже умеет принимать телефонию двумя способами:

- `POST /webhooks/telephony`
  Прямой webhook для провайдера телефонии.

- `POST /webhooks/external/telephony`
  Универсальный webhook через общий integration-маршрут.
...
- `TELEPHONY_AUTH_HEADER=X-Telephony-Secret`
- `TELEPHONY_AUTO_START_VOICE_AGENT=false`
- `TELEPHONY_ALLOWED_EVENTS_JSON`
- `TELEPHONY_LINE_ROUTING_JSON`

### Как работает защищённый webhook

Если задан `TELEPHONY_WEBHOOK_SECRET`, каждый запрос на `/webhooks/telephony`
должен передавать этот секрет в заголовке из `TELEPHONY_AUTH_HEADER`.

Пример:

```http
POST /webhooks/telephony
X-Telephony-Secret: your-secret
...
```json
{
  ...
  "line_name": "Основная линия",
  "campaign": "spring-calls"
}
```

### Когда включать voice agent

- Для ручного запуска из телефонии передай `use_voice_agent=true`.
- Для автоматического старта на входящих звонках включи `TELEPHONY_AUTO_START_VOICE_AGENT=true`.
- Статус интеграции и активный режим можно проверить через `GET /api/telephony/status` или `GET /status`.

### Маршрутизация по линиям

`TELEPHONY_LINE_ROUTING_JSON` позволяет связать номер/линию с кампанией или командой.
