...
def _load_json_list(env_name: str) -> list:
    ...
    if not isinstance(data, list):
        logger.warning("%s must be a JSON array and will be ignored", env_name)
        return []
    return data
...
class Settings:
    ...
    VOICE_STT_PROVIDER: str = os.getenv("VOICE_STT_PROVIDER", "payload_text")
    VOICE_TTS_PROVIDER: str = os.getenv("VOICE_TTS_PROVIDER", "external_tts")
    VOICE_FEMALE_VOICE: str = os.getenv("VOICE_FEMALE_VOICE", "female-natural-ru")
    VOICE_AGENT_NAME: str = os.getenv("VOICE_AGENT_NAME", "Виртуальный менеджер")
    VOICE_HANDOFF_MESSAGE: str = os.getenv(
        "VOICE_HANDOFF_MESSAGE",
        "Передаю ваш звонок живому менеджеру отдела продаж.",
    )
    TELEPHONY_ENABLED: bool = os.getenv("TELEPHONY_ENABLED", "true").lower() == "true"
    TELEPHONY_PROVIDER: str = os.getenv("TELEPHONY_PROVIDER", "telephony")
    TELEPHONY_WEBHOOK_SECRET: str = os.getenv("TELEPHONY_WEBHOOK_SECRET", "")
    TELEPHONY_AUTH_HEADER: str = os.getenv("TELEPHONY_AUTH_HEADER", "X-Telephony-Secret")
    TELEPHONY_AUTO_START_VOICE_AGENT: bool = (
        os.getenv("TELEPHONY_AUTO_START_VOICE_AGENT", "false").lower() == "true"
    )
    TELEPHONY_ALLOWED_EVENTS: list = _load_json_list("TELEPHONY_ALLOWED_EVENTS_JSON") or [
        "incoming_call",
        "call_started",
        "call_finished",
        "missed_call",
    ...
    TELEPHONY_LINE_ROUTING: dict = _load_json_map("TELEPHONY_LINE_ROUTING_JSON")
    ...
    DELIVERY_RATE_PER_KM: float = 35
    DELIVERY_BASE_FEE: float = 650
    DELIVERY_MIN_CHARGED_VOLUME: int = 6
...
if settings.TELEPHONY_ENABLED and settings.TELEPHONY_WEBHOOK_SECRET:
    logger.info(
        "Telephony webhook protection is enabled via header %s",
        settings.TELEPHONY_AUTH_HEADER,
    )
